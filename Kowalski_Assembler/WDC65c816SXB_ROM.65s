...
; Definitions used in sending info about the monitor (debug command 4).
BOARD_ID                    = $58
WORK_RAM_START              = $7E00
SHADOW_VECTOR_START         = $7EE4
SHADOW_VECTOR_SIZE          = $01C
MONITOR_CODE_START          = $8000
CPU_VECTOR_START            = $FFE4
HW_IO_START                 = $7F00
HW_IO_SIZE                  = $0100
HW_BRK_ADDR                 = $FFFFFF
...
Pointer_Table:
    ...
    ; Used entries in the pointer table.
        .WORD   Signature_String
        .WORD   Initialize_System
        .WORD   Is_VIA_USB_RX_Data_Avail
        .WORD   Sys_VIA_USB_Char_RX
        .WORD   Sys_VIA_USB_Char_TX
        .WORD   RAM_IN_MONITOR
        .WORD   Monitor_Version_String
        .WORD   IRQ_02_ENTRY_VECTOR
        .WORD   IRQ_02_Entry_Vector_Default
        .WORD   NMI_02_Entry_Vector_Default
        .WORD   BRK_816_Entry_Vector_Default
        .WORD   NMI_816_Entry_Vector_Default
...
Dbg_Cmd_2_Write_Mem:
        ...
        jsr     Sys_VIA_USB_Char_RX
        sta     \2DBG_VAR_00	; 16-bit addr LSB
        jsr     Sys_VIA_USB_Char_RX
        sta     \2DBG_VAR_01	; 16-bit addr MSB
        jsr     Sys_VIA_USB_Char_RX
        STA     \2DBG_VAR_02	; bank
...
Dbg_Cmd_3_Read_Mem:
        ; Read the 24-bit starting address, LSB-first.
        jsr     Sys_VIA_USB_Char_RX
        sta     \2DBG_VAR_00	; 16-bit LSB
        jsr     Sys_VIA_USB_Char_RX
        sta     \2DBG_VAR_01	; Page
        jsr     Sys_VIA_USB_Char_RX
        STA     \2DBG_VAR_02	; Bank

        ; Read the 16-bit byte count, LSB-first.
        jsr     Sys_VIA_USB_Char_RX
        sta     \2DBG_VAR_03	; Count LSB
        jsr     Sys_VIA_USB_Char_RX
        STA     \2DBG_VAR_04	; Count MSB
...
Dbg_Cmd_4_Sys_Info:
        ...
        lda     #<MONITOR_CODE_START
        jsr     Sys_VIA_USB_Char_TX
        lda     #>MONITOR_CODE_START
        jsr     Sys_VIA_USB_Char_TX
        lda     #{MONITOR_CODE_START>>16}
        JSR     Sys_VIA_USB_Char_TX
        ...
        LDA     #>SHADOW_VECTOR_START
        JSR     Sys_VIA_USB_Char_TX
        lda     #{SHADOW_VECTOR_START>>16}
        JSR     Sys_VIA_USB_Char_TX

        ; Shadow vector end address + 1.
        lda     #<{SHADOW_VECTOR_START + SHADOW_VECTOR_SIZE}
        jsr     Sys_VIA_USB_Char_TX
        lda     #>{SHADOW_VECTOR_START + SHADOW_VECTOR_SIZE}
        jsr     Sys_VIA_USB_Char_TX
        LDA     #{{SHADOW_VECTOR_START + SHADOW_VECTOR_SIZE}>>16}
        jsr     Sys_VIA_USB_Char_TX

        ; Send the hardware vector address.
        lda     #<CPU_VECTOR_START
        jsr     Sys_VIA_USB_Char_TX
        lda     #>CPU_VECTOR_START
        jsr     Sys_VIA_USB_Char_TX
        LDA     #{CPU_VECTOR_START>>16}
        jsr     Sys_VIA_USB_Char_TX
        ...
        lda     #<{HW_IO_START + HW_IO_SIZE - 1}
        jsr     Sys_VIA_USB_Char_TX
        lda     #>{HW_IO_START + HW_IO_SIZE - 1}
        jsr     Sys_VIA_USB_Char_TX
        LDA     #{{HW_IO_START + HW_IO_SIZE - 1}>>16}
        jsr     Sys_VIA_USB_Char_TX
...
.was_in_native_mode:
        ...
        ; Set XY to 16-bit mode, and A to 8-bit mode.
        SEP     #$20
        REP     #$10
...
.branch:
        ...
        LDA     #$00
        .DB     $00 ; Single-byte BRK instruction.
        jmp     Sys_VIA_USB_Char_TX
...
IRQ_02_Entry_Vector_Default:
        ...
        lda     #$00
        STA     \3RAM_IN_MONITOR	; No longer in ROM monitor code.
...
.enter_native_mode:
        ...
        ; Set XY and A to 8-bit mode.
        SEP     #$30

        ; Save the reason we're entering the monitor into work-RAM.
        STA     \3RAM_ENTER_MONITOR_REASON
...
Save_Context_Enter_Monitor:
        ...
        ; Set the direct-page register to page 0.
        lda     \2#$00
        TCD
...
RSVD_FFE0:  .WORD   $FFFF               ; $FFE0
RSVD_FFE2:  .WORD   $FFFF               ; $FFE2
COP_816:    .WORD   COP_816_entry       ; $FFE4
BRK_816:    .WORD   BRK_816_entry       ; $FFE6
ABORT_816:  .WORD   ABORT_816_entry     ; $FFE8
NMI_816:    .WORD   NMI_816_entry       ; $FFEA
RSVD_FFEC:  .WORD   Infinite_Loop_2     ; $FFEC
IRQ_816:    .WORD   IRQ_816_entry       ; $FFEE

        ; 65C02 Emulation-Mode Vectors
RSVD_FFF0:  .WORD   Infinite_Loop       ; $FFF0
RSVD_FFF2:  .WORD   Infinite_Loop       ; $FFF2
COP_02:     .WORD   COP_02_entry        ; $FFF4
RSVD_FFF6:  .WORD   Infinite_Loop       ; $FFF6
ABORT_02:   .WORD   ABORT_02_entry      ; $FFF8
NMI_02:     .WORD   NMI_02_entry        ; $FFFA
RESET:      .WORD   RESET_entry         ; $FFFC
IRQ_02:     .WORD   IRQ_02_entry        ; $FFFE